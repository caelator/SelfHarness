...
def main(argv: list[str] | None = None) -> int:
    ...
    if not manifest.is_file():
        raise SystemExit(f"manifest path does not exist: {manifest}")
    ...
    try:
        ...
        print(stable_json_dumps({"ok": False, "reason": "provenance-signing-error", "message": str(exc)}))
        return 2
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sign exact release provenance manifest bytes.")
