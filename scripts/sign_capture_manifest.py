...
def main(argv: list[str] | None = None) -> int:
    ...
    output = args.out.resolve() if args.out is not None else manifest.with_name(manifest.name + ".sig")
    try:
        load_capture_manifest(manifest)
        manifest_bytes = manifest.read_bytes()
        sidecar = _build_sidecar(args, manifest=manifest, manifest_bytes=manifest_bytes)
    except ExternalSignerError as exc:
        print(stable_json_dumps(exc.failure.to_jsonable()), file=sys.stderr)
        return 2
    except (CorpusSigningError, OSError, CaptureManifestError) as exc:
        payload = {
            "schema_version": "1.0",
            "ok": False,
            "reason": "capture-manifest-signing-error",
            "message": str(exc),
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sign exact capture manifest bytes.")
    parser.add_argument("--manifest", type=Path, required=True, help="Operator capture manifest path.")
    ...
    parser.add_argument("--out", type=Path, help="Signature sidecar path; defaults to MANIFEST.sig.")
    _add_passphrase_args(parser)
    return parser


def _build_sidecar(args: argparse.Namespace, *, manifest: Path, manifest_bytes: bytes) -> dict[str, object]:
    if args.external_signer is not None:
        return _external_sidecar(args, manifest=manifest, manifest_bytes=manifest_bytes)
    if args.private_key is None:
        raise CorpusSigningError("--private-key is required unless --external-signer is used")
    return _local_sidecar(args, manifest=manifest, manifest_bytes=manifest_bytes)


def _local_sidecar(args: argparse.Namespace, *, manifest: Path, manifest_bytes: bytes) -> dict[str, object]:
...
def _external_sidecar(args: argparse.Namespace, *, manifest: Path, manifest_bytes: bytes) -> dict[str, object]:
...
def _sidecar(
    *,
    ...
    verify_bytes_signature(manifest_bytes, signature_b64, public_key_b64)
    return {
