...
4. Leave the P107 split-outcome lineage loop's multi-commit skip behavior intact. A merged harness hash proves the next harness state in hash space; it does not derive a merged split-outcome aggregate from individual candidate rows.
...
6. If future planned stubs include multi-commit-shaped rounds, they must carry a deterministic `harness_after_merged_sha256`; the default one-commit rehearsal shape can remain unchanged.

7. Rotate only serialized fixture hashes whose payloads actually change. The canonical audit hash and canonical LLM audit hash must not change because the engine default output path is unchanged.

8. Add tests covering: (a) valid merged-hash continuity across multi-commit rounds; (b) rejection when the next round's `harness_before_sha256` disagrees with the prior merged hash; (c) rejection when `harness_after_merged_sha256` appears on a single-commit round; (d) rejection when a new-style multi-commit round with harness hashes omits it; (e) legacy reduced bundles without harness hashes still verify.
...
- `src/self_harness/reproduction_bundle.py` — extend `_cross_artifact_proposal_validation_binding` to enforce merged-hash continuity on multi-commit rounds in the harness-hash path; leave split-outcome multi-commit continuity explicitly skipped.
...
- `src/self_harness/capture_manifest_build.py` — future multi-commit planned stubs must carry the merged-hash field; the current default one-commit stubs do not need to change.
...
- `make readiness`, `make release-candidate-evidence`, `make capture-manifest-check`, and `make reproduction-readiness-check` pass; fixture hashes rotate only if their serialized payloads change.
