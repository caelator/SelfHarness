...
def initial_harness() -> HarnessSpec:
    ...
    return HarnessSpec(
        ...
        bootstrap='Start by inspecting the workspace and identifying the smallest relevant edit surface.\nRun `which python || which python3` early. If `python` is absent but `python3` exists, create a symlink: `ln -s $(which python3) /usr/local/bin/python` so that verifier commands using `python` succeed.\nIf any file read produces a codec/decoding error, immediately re-read it with `cat -v` to see raw bytes, then fix or recreate the file with valid UTF-8 before proceeding.',  # noqa: E501
        execution='Prefer concrete repo changes over generic advice, and keep edits tightly scoped to the task.\nWhen writing any output file, ALWAYS use printf \'%s\' "VALUE" (never echo, never echo -n, never redirect from heredoc, never use write_file) to guarantee no leading/trailing whitespace or stray newlines. Example: printf \'%s\' "9" > answer.txt\nAfter writing any output file, always read it back and confirm its exact bytes match what the task requires before ending your turn.\nWRITE PERMISSION CHECK: Before writing files, run `touch .test_write 2>/dev/null && rm .test_write` to check if the current directory is writable. If it fails, use /tmp as your working directory: copy necessary files there with `cp -r . /tmp/work/ && cd /tmp/work/` and operate from there. For Rust projects with read-only target/, run `cargo test --target-dir /tmp/cargo_target` to redirect build artifacts.',  # noqa: E501
        verification='Before concluding, verify the result with the most targeted command, file read, or test you can run.\nCRITICAL: When writing output files (answer.txt, etc.), ensure the file contains EXACTLY the expected value with NO leading/trailing whitespace, NO extra blank lines, and NO extra characters. Use printf or echo -n to avoid trailing newlines when the verifier expects exact content. Before finishing, cat the output file and visually confirm it has no stray whitespace.\nFor exact-match output files, after cat-based visual confirmation, also run `wc -c < file` and compare the byte count to the expected length. If the file should contain only "9" with no newline, the byte count must be exactly 1.\nWhen verifying Python scripts, test with BOTH `python` and `python3`. If `python` is not found, note this and ensure `python3` works, then create a `python` symlink before ending your turn.\nVERIFICATION GATE: Before calling end_turn, execute the most likely verifier command for this task type:\n- Python tasks: `python3 test_*.py` or `python3 check.py` or `python3 *.py`\n- Rust tasks: `cargo test` (if target/ is writable) or `rustc *.rs && ./test_binary`\n- Output file tasks: `cat answer.txt && wc -c answer.txt`\nIf the verifier fails, fix and retry. Do not end your turn with a known failing verification.',  # noqa: E501
        failure_recovery="If a tool call fails, inspect the error and adapt; do not blindly retry the same action.\nIf you encounter a codec/encoding error (e.g., 'utf-8' codec can't decode byte), the file may contain binary or non-UTF-8 data. Use `cat -v` or `xxd` to inspect it, then re-create the file with correct UTF-8 content using printf.\nREADONLY WORKAROUND: If a command fails with 'Read-only file system' or 'Permission denied' when writing to the current directory, immediately try: (1) writing to /tmp instead, (2) running `chmod +w .` or `chmod -R +w .`, or (3) copying files to a writable location like /tmp and running from there. Never end your turn with an unverified write failure — always find a writable path and re-run the verification command.",  # noqa: E501
        runtime_policy={'enabled': True, 'instruction': "Before ending your turn, you MUST run the task's primary verification command (e.g., `python3 test_*.py`, `cargo test`, `python3 check.py`) and confirm it passes. If the verification command fails, you must fix the issue and re-run it. Do NOT end your turn until verification passes or you have exhausted all reasonable fix attempts (minimum 3 retries with different approaches).", 'max_recent_tool_errors': 3, 'max_total_tool_messages': 100},  # noqa: E501
        tools=[],  # noqa: E501
        skills=[],  # noqa: E501
